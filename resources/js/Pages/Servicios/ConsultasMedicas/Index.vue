    ...
    if (pagoForm.value.metodo_pago === "qr") {
        ...
        if (!pagoQrVerificando.value && !pagoQrEsperando.value) {
            ...
            if (!pagoQrVerificacionLista.value) {
                displayToast("danger", "Pago aún no confirmado. Pulse «Verificar pago» cuando el cliente haya pagado.");
            }
        }
        return;
    }
                ...
                <div class="space-y-4">
                    ...
                    <div v-if="pagoForm.metodo_pago === 'qr'">
                        ...
                        >
                            ...
                            />
                            <p v-if="pagoQrEsperando || pagoQrVerificando" class="text-sm text-emerald-700 mt-3">
                                {{ textoEstadoVerificacionQr() }}
                            </p>
            ...
            <template #footer>
                <div class="flex justify-end w-full gap-2">
                    ...
                    <FwbButton
                        ...
                        color="green"
                        :loading="loading || pagoQrVerificando || pagoQrEsperando"
                        :disabled="pagoQrVerificando || pagoQrEsperando"
                    >
                        {{
                            pagoForm.metodo_pago === "qr" && !pagoQrImage
                                ? "Generar QR y cobrar"
                                : pagoForm.metodo_pago === "qr" && pagoQrEsperando
                                  ? "Espere 30 segundos..."
                                  : pagoForm.metodo_pago === "qr" && pagoQrVerificando
                                    ? "Consultando pago..."
                                    : pagoForm.metodo_pago === "qr" && pagoQrImage && !showConfirmacionPago
                                      ? "Verificar pago"
                                      : "Registrar pago"
                        }}
                    </FwbButton>
